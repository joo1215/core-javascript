...
function first(){
  ...
  // return second  // -> 함수 자체가 실행  /  이유 : js 함수는 = 값
...
// closure를 사용하는 이유 : 전역의 오염을 막기 위해
...
//closure를 사용했을 때
// 아래 함수는 다른애들이 못들어오게 밀폐를 시킨것과도 같다 => closure
function number(){
  let count = 0;
...
//const는 전역에 선언해도 오염되지 않는다고 보는건가요? - 여기서는 오염이 된 것, 그러나 다론 모듈 안에 들어가게 된다면 아님(?)
// 이렇게하면 counter는 함수의 역할을 할 수 있음
// 대신 console.log(count)는 접근 할 수 없음 -> 오염이되지않았다(?)
//그럼 number함수 전체를 클로저라고 생각하면 될까요 아니면 number함수를 counter에 할당한 것을 클로저로 이해하면 될까요?  -  number함수를 counter에 할당했을 때 number 함수가 클로저의 역할을 한다 라고 이해했어요
